...
def ht(train_features, train_labels, test_features, test_labels):
...
def S2T_p_hh(train_features, train_labels, test_features, test_labels):
    ...
    test_features = ht(train_features, train_labels, test_features, test_labels)
...
def cv_nfold_blc(func, all_data, nfold = 1000):
    ...
    for index,permu in enumerate(data_permu):
        ...
        X_source = [[val[1]
            for val in X_i_source[math.ceil(len(X_i_source)/nfold)*thresh:math.ceil(len(X_i_source)/nfold)*(1+thresh)]]
        ...
        X_target = [[val[1]
            for val in X_i_target[math.ceil(len(X_i_target)/nfold)*thresh:math.ceil(len(X_i_target)/nfold)*(1+thresh)]]
                    for thresh in range(nfold) 
        ]
...
if __name__ == '__main__':
    ...
    print('lowerbound score:',round(np.mean(S2T_scores),3))
    print('Domain Space Alignment model score:',round(np.mean(func_scores),3))
